...
async function createProduct({ name, description, price, category, stock_type }, file) {
  if (!usesSupabase()) {
...
async function updateProduct(id, { name, description, price, category, stock_type }, file) {
  if (!usesSupabase()) {
    const products = readLocalProducts();
    const index = products.findIndex(p => p.id === id);
